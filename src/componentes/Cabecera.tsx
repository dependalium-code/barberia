    ...
    <header className="campo-tinta sticky top-0 z-50 bg-tinta text-acero-05">
      <div className="mx-auto flex h-14 max-w-[86rem] items-stretch gap-4 px-4 sm:px-6 lg:px-10">
        ...
        >
          ...
          <span className="cota hidden text-acero-30 sm:block">{NEGOCIO.ciudad}</span>
        </Link>

        <nav aria-label="Principal" className="ml-auto hidden items-stretch lg:flex">
          {ENLACES.map((e) => (
            <Link
              key={e.href}
              href={e.href}
      ...
      >
        <nav aria-label="Principal (móvil)" className="px-4 pb-6 pt-2 sm:px-6">
          {ENLACES.map((e) => (
            <Link
              key={e.href}
              href={e.href}
